...
def compute_metrics(results: list[dict]) -> dict:
    ...
    has_disclaimer, has_citation, latency_ms；可选 ``grounded``（有据率裁判
    打分 0-1，未评测为 ``None``，如被拦截或无引用的用例）。
    ...
    ``groundedness`` 为裁判打分的均值（仅统计已评测的用例）。
    """
    ...
    grounded_scores = [
        float(r["grounded"]) for r in answered if r.get("grounded") is not None
    ]
